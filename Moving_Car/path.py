...
class Path:
	...
	def generate_midpoint_path(self, pp):
		...
		if (len(pp.cone.visible_cone_list[Side.LEFT]) > 1 
		and len(pp.cone.visible_cone_list[Side.RIGHT]) > 1 
		and (pp.cone.new_visible_cone_flag[Side.LEFT] == True or pp.cone.new_visible_cone_flag[Side.RIGHT] == True)): #track_number == 0 and
			...
			for left_cone in pp.cone.visible_cone_list[Side.LEFT]:
				for right_cone in pp.cone.visible_cone_list[Side.RIGHT]:
			...
			for i in range(len(path_midpoints[0])):
			  #  print(i)
				...
				if dist_car < pp.car.fov / pp.ppu:
					
					a_b = Vector2(path_midpoints[0][i],path_midpoints[1][i]) - pp.car.position
					...
					if np.abs(alpha) < pp.car.fov_range:
