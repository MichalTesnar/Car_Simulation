...
def render(pp, fullscreen = False):
    ...
    for category in Side:
        ...
        for cone in pp.cone.visible_cone_list[category]:
            if cone.in_fov:
                draw_line_dashed(pp.screen, (150,150,150),(pos_1,pos_2) , cone.position * pp.ppu , pp.view_offset, width = 1, dash_length = 10, exclude_corners = True)
    ...
    if pp.cone.first_visible_cone[Side.LEFT] != 0 and pp.cone.first_visible_cone[Side.RIGHT] != 0:
        draw_line_dashed(pp.screen, (255, 51, 0),(pp.cone.first_visible_cone[Side.LEFT].position.x* pp.ppu, pp.cone.first_visible_cone[Side.LEFT].position.y* pp.ppu) , (pp.cone.first_visible_cone[Side.RIGHT].position.x* pp.ppu, pp.cone.first_visible_cone[Side.RIGHT].position.y* pp.ppu), pp.view_offset , width = 2, dash_length = 5, exclude_corners = True)
   ...
   # if path_midpoints != 0 and len(path_midpoints) > 0:
    ...
    if fullscreen == False:
     ...
     #   pp.screen.blit(text_surf, text_pos)
        ...
        text_surf = text_font.render(f'Steering : {round(pp.car.steering,1)}', 1, (255, 255, 255))
        text_pos = [10, 35]
        pp.screen.blit(text_surf, text_pos)
